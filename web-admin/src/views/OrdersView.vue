  ...
  <section>
    <div class="page-head">
      <div>
        ...
        <p class="muted">查看订单、筛选状态，并更新订单流转状态。</p>
      </div>
...
async function updateStatus(order, status) {
  errorMessage.value = '';
  try {
    await http.patch('/admin/orders/' + order.id + '/status', { status });
    await fetchOrders();
  } catch (error) {
...
.row-actions { flex-wrap: wrap; justify-content: flex-end; max-width: 280px; }
.danger-card { margin-bottom: 14px; }
</style>
